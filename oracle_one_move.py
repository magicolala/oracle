...
def clean_pgn(pgn):
    ...
    # Todo : Check if this line can be replace by `[*headers, "", " ".join(moves)]`
    cleaned_lines = headers + [""] + [" ".join(moves)]  # noqa: RUF005
                    ...
                    if "O-O" in legal_moves and "O-O-O" in legal_moves:
                        if combined_sequence == "O-O":
                            response_after_castling = make_api_call(
                                prompt + " " + combined_sequence, retries
                            )
...
def process_pgn_and_rating(
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_top_sequences = executor.submit(
            get_top_sequences, prompt, legal_moves, model, depth, 3, prob_threshold
        )
    ...
    highest_1500, lowest_1500, second_highest_1500, second_lowest_1500 = find_win_percentages(
        win_percentages_1500
    )
    ...
    intercept_best_move = {
        "classical": (((min(rating, 4100)) / 4100) + (20 * (best_move_importance / 100) ** 0.5))
        * (min(rating, 4100))
        / 4100,
    ...
    multiplier = {"classical": 150, "rapid": 500, "blitz": 1000, "bullet": 4000}.get(game_type, 150)
    ...
    for _, (_, _, new_norm_prob, eval_score) in enumerate(new_normalized_moves):
    ...
    new_normalized_moves = [
        (
            move,
            prob,
            (prob / valid_total_probability) * 100 if valid_total_probability > 0 else 0.0,
            eval_score,
        )
        for move, prob, _, eval_score in new_normalized_moves
    ]

    rows_to_print = list(range(min(3, len(new_normalized_moves))))
    best_move_idx_in_normalized = next(
        i for i, (move, _, _, _) in enumerate(new_normalized_moves) if move == best_move
    )
    ...
    for i in rows_to_print:
        ...
        percentage_loss = percentage_losses.get(move, 0)
        is_best_move = move == best_move
        best_move_notation = get_best_move_notation(
            win_percentage, new_norm_prob, board.turn, current_win_percentage
        )
        ...
        row = [
            f"#{i+1}",
            colored_move,
            f"{new_norm_prob:.2f}%" if raw_prob > 0 else "0.00%",
            f"{win_percentage:.2f}%",
        ]
    ...
    logging.debug(f"Total cost: ${total_cost:.6f}")
...
if __name__ == "__main__":
    logging.debug('Enter the PGN (type "END" on a new line to finish):')
    pgn_content = []
    while True:
        line = input()
        if line.strip() == "END":
